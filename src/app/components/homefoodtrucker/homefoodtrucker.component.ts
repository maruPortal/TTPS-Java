...
@Component({
  selector:
    'app-homefoodtrucker, ngbd-dropdown-basic, ngbd-carousel-pause, ngbd-nav-markup',
  ...
  aceptarReserva(soli) {
    this.userService.modificarSolicitud(soli.id, 'Aceptada').subscribe(
      (response) => {
        ...
        this.toastr.success(
          "Se agrego la reserva a la solapa 'En Curso'",
          'Solicitud Aceptada'
        );
      },
      (err: HttpErrorResponse) => {
        console.log('estado de error: ' + err.status);
        if (err.status == 400) {
          this.toastr.error('La solicitud fue cancelada', 'Error', {
            timeOut: 4000,
          });
          this.nuevas.splice(this.nuevas.indexOf(soli), 1);
        } else {
  ...
  finalizarReserva(soli) {
    this.userService.modificarSolicitud(soli.id, 'Finalizada').subscribe(
      (response) => {
        ...
        this.toastr.success(
          'Se finalizo la reserva correctamente y se la habilito para Calificar',
